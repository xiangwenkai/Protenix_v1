...
from protenix.model.eclip_binding import EclipBindingScorer, EclipSignalLoss
...
def test_eclip_sidecar_state_is_not_inside_model_state_dict():
    ...
    scorer = EclipBindingScorer(learn_temperature=True)
    signal_loss = EclipSignalLoss()

    checkpoint = build_eclip_ppft_checkpoint(
        model=model,
        binding_scorer=scorer,
    ...
    assert "eclip_binding_scorer" in checkpoint
