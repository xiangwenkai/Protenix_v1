...
def _import_pxmeter_builder(repo_root: Path):
    script = repo_root / "examples/sft/run_pxmeter_other_targets_inference.py"
    return _load_module(script, "pxmeter_other_targets_inference_for_foldbench")


def _generate_pxmeter_inputs(fold_cfg: Any, work_dir: Path, repo_root: Path) -> Path:
    px = _import_pxmeter_builder(repo_root)
    args = type("Args", (), {})()
    args.targets_dir = _cfg(
        fold_cfg,
        "targets_dir",
        "/inspire/ssd/project/sais-bio/public/xiangwenkai/GITHUB/PXMeter/targets",
    )
...
def prepare_validation_input(fold_cfg: Any, eval_dir: Path, repo_root: Path) -> Path:
...
def _prediction_rows(
    ...
    dockq_gt_dir = Path(_cfg(fold_cfg, "dockq_ground_truth_dir")).resolve()
    ...
    for task in tasks:
        ...
        gt_path = gt_dir / f"{sample_name}.cif"
        if gt_path.exists():
        ...
        for candidate in (
            ...
            if candidate.exists():
                _link_or_copy(candidate, dockq_eval_gt_dir / f"{sample_name}.cif")
                break
...
def run_foldbench_validation(
    ...
    input_json = prepare_validation_input(fold_cfg, eval_dir, repo_root)
